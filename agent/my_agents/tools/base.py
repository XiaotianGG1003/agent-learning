...
class Tool(ABC):
    ...
    def to_openai_schema(self) -> Dict[str, Any]:
        ...
        for param in parameters:
            # 类型映射：将工具参数类型映射为 JSON Schema 允许的类型
            prop_type = self._map_parameter_type(param.type)
...
class AutoGeneratedTool(Tool):
    ...
    def _extract_description_from_docstring(self) -> str:
        ...
        if not doc:
            return f"提取描述 {self.method.__name__}"
    ...
    def _python_type_to_tool_type(self, py_type) -> str:
        ...
        # 解包 Optional[X] / Union[X, None]：带 None 默认值的参数会被 get_type_hints
        # 包成 Optional，若不解包则 List[str]=None 这类会被误判为 string。
        if origin is Union:
            non_none = [arg for arg in getattr(py_type, '__args__', ()) if arg is not type(None)]
            if len(non_none) == 1:
                return self._python_type_to_tool_type(non_none[0])
            return "string"
