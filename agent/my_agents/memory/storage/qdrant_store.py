...
class QdrantVectorStore:
    ...
    def _build_filter(self, where: Optional[Dict[str, Any]] = None) -> Optional[Filter]:
        """将简单 payload 条件转换为 Qdrant Filter。"""
        if not where:
            return None
    ...
    def delete_by_filter(self, where: Optional[Dict[str, Any]] = None) -> bool:
        """
        按 payload 条件删除向量。

        Args:
            where: 简单等值过滤条件，例如 {"namespace": "project_a"}
        ...
        try:
            ...
            if query_filter is None:
                logger.warning("⚠️ delete_by_filter 未提供过滤条件，拒绝执行")
                return False
    ...
    def count_vectors(self, where: Optional[Dict[str, Any]] = None) -> int:
        """按 payload 条件统计向量数量；不传 where 时统计整个集合。"""
        try:
            query_filter = self._build_filter(where)
    ...
    def get_collection_stats(self, where: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        获取集合统计信息（兼容抽象接口）
        """
        ...
        if where:
            total_points = info.get("points_count", info.get("vectors_count", 0))
            filtered_count = self.count_vectors(where)
            info["total_points_count"] = total_points
            info["points_count"] = filtered_count
            info["vectors_count"] = filtered_count
            info["filter"] = where
            info["scope"] = "filtered"
